...
class Jira:
    def __init__(self, creds):
        ...
        try:
            ...
            self.conn = jira.client.JIRA(server=endpoint, basic_auth=(username, token))
        except Exception as e:
            print(f"Error connecting to JIRA: {e}")
            return False


    def create_jira_issues_from_file(self, filename, username):
        """
        ...
        for i, word in enumerate(lines, 1):
            ...
            try:
                ...
                jql = f'project = {self.project_key} AND summary ~ "{word}"'
                existing = self.conn.search_issues(jql, maxResults=1)
                if existing:
            ...
            try:
                ...
                if username != None:
                    assignee_id = self.conn.search_users(query=username)[0].accountId
                    issue_dict['assignee'] = {'id': assignee_id}

                new_issue = self.conn.create_issue(fields=issue_dict)
...
def parse_arguments():
    ...
    parser.add_argument(
        '--username',
        help='email of the user to assign the tickets to'
    )
...
if __name__ == '__main__':
    ...
    myjira = Jira(credentials_handler)
    success = myjira.create_jira_issues_from_file(args.hypervisors_file, args.username)
